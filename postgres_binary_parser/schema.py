...
class Schema:
    ...
    def union(cls, schemas, with_prefix=False, drop=None, add=None, rename=None, schema_name=None, options=None):
        ...
        for schema in schemas:
            for col in schema.cols:
                ...
                if with_prefix:
                    prefix = schema.options.get('prefix', schema.name)
    ...
    def conform_df(self, df, storage_target='pandas', skip_sort=False, add_prefix=False):
        ...
        if self.options.get('reset_index', False):
...
class PartialSchema(Schema):
    ...
    def conform_df(self, df, storage_target='pandas', skip_sort=False, add_prefix=False):
        ...
        if not set(df.columns) >= set(self.col_names()):
            ...
            raise TypeError('some columns missing from df: {}'.format(missing))

        for col in self._inferred_cols(df, skip_cols=self.cols):
            if isinstance(col, Column):
                col.conform(df, storage_target=storage_target)
    ...
    def validate_schema(self, other_schema, validate_options=False):
        if not set(other_schema.col_names()) >= set(self.col_names()):
            missing = set(self.col_names()) - set(other_schema.col_names())
            raise TypeError('schema missing required columns: {}'.format(missing))

        other_cols = set(other_schema.cols)
        for col in self.cols:
            if col not in other_cols:
                raise TypeError('required col {} is missing from schema'.format(repr(col)))
        
        if validate_options:
            for k, v in self.options.items():
                if k not in other_schema.options or other_schema.options[k] != v:
                    raise TypeError('schema missing option {}: {}'.format(repr(k), repr(v)))
        return
...
class MultiSchema:
    def __init__(self, schema_dict):
        ...
        for name, schema in schema_dict.items():
            if isinstance(schema, MultiSchema):
                continue
            if name != schema.name:
                raise TypeError('MultiSchema keys must match schema names. Nonmatching: {}: {}'.format(
                    repr(name), repr(schema)
                ))
    ...
    @classmethod
    def merge(cls, multischemas):
        schema_dict = {}
        for ms in multischemas:
            schema_dict.update(ms.schema_dict)
...
class Column(metaclass=ColumnMeta):
    ...
    def __repr__(self):
        options = ', '.join('{}={}'.format(k, v) for k, v in sorted(self.options.items()))
        if options:
            options = ', '+options
        return '{type}("{name}"{options})'.format(type=self.__class__.__name__, name=self.name, options=options)
...
def _(col):
    return hasattr(col, 'cat') and col.cat.categories.dtype == 'object'
...
def _(col):
    ...
    if len(cats) > 0:
        col = reduce_categories(str, col)
    else:
        col = col.cat.rename_categories(cats.astype('object'))
    return col
...
def _(col):
    ...
    to_dt = partial(pandas.to_datetime, errors='coerce', infer_datetime_format=True)
    cat_dt_col = reduce_categories(to_dt, cat_col, vectorized=True)
    return pandas.to_datetime(cat_dt_col)
...
def _(col):
    cat_col = col.astype('category')
    cat_dt_col = reduce_categories(pandas.to_timedelta, cat_col, vectorized=True)
    return pandas.to_timedelta(cat_dt_col)
